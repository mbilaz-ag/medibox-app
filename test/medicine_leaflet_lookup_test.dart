...
void main() {
  ...
  test('selects an exact medicine and extracts leaflet sections', () async {
    final client = MockClient((request) async {
      if (request.url.path.startsWith('/paieska/')) {
        return http.Response('''<div class="card__title"><a href="/espumisan-240mg.html">Espumisan 240 mg minkštosios kapsulės</a></div>
  ...
  test('does not confuse 40 mg with 240 mg', () async {
    ...
    expect(() => MedicineLeafletLookup(client).find(medicine), throwsFormatException);
  });
}
